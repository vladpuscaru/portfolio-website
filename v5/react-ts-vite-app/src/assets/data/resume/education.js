import logoUU from "../../common/icons/logo_uu.png";
import logoCNIV from "../../common/icons/logo_cniv.png";
import logoASE from "../../common/icons/logo_ase.png";
import logoSTEPIT from "../../common/icons/logo_stepit.png";

export const education = [
    {
        university: {
            name: "Utrecht University",
            logo: logoUU,
            faculty: "Game and Media Technology"
        },
        degree: "Master's Degree",
        startDate: "2023",
        endDate: "Present",
        description: "<ul>" +
            "<li>I am back to school in order to specialise and research in game development-related areas such as <strong>graphics</strong>, <strong>physics</strong>, <strong>crowd simulation</strong></li>" +
            "</ul>"
    },
    {
        university: {
            name: "STEP IT Academy Bucharest",
            logo: logoSTEPIT,
            faculty: "Software Development"
        },
        degree: "Private education",
        startDate: "2018",
        endDate: "2019",
        description: "<ul>" +
            "<li>Software Development courses at a private institution</li>" +
            "<li>Curricula included: <strong>C</strong>, <strong>C++</strong>, <strong>C#</strong>, <strong>Algorithms</strong>, and <strong>Design Patterns</strong></li>" +
            "</ul>"
    },
    {
        university: {
            name: "Academy of Economic Studies, Bucharest",
            logo: logoASE,
            faculty: "Economic Informatics"
        },
        degree: "Bachelor's Degree",
        startDate: "2017",
        endDate: "2020",
        description: "<ul>" +
            "<li>Final score of 10/10, with the thesis \"IoT Solution for Smart Home\"</li>" +
            "</ul>"
    },
    {
        university: {
            name: "National College 'Ienachita Vacarescu', Targoviste",
            logo: logoCNIV,
            faculty: "Social Scenes"
        },
        degree: "Baccalaureate Diploma",
        startDate: "2012",
        endDate: "2016",
        description: ""
    },
]